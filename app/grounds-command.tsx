...
// Public aliases for the route entry points (see app/command-entry.tsx).
export type CommandProfileMode = ProfileMode;
export type CommandView = View;
type HomeActionItem = {
  id: string;
  title: string;
  detail: string;
...
export default function GroundsCommand({ ownerName, todayLabel, routeMode, routeView }: { ownerName: string; todayLabel: string; routeMode?: CommandProfileMode; routeView?: CommandView }) {
  const [state, setState] = useState<AppState>(INITIAL_STATE);
  const [loaded, setLoaded] = useState(false);
  const [saveStatus, setSaveStatus] = useState<SaveStatus>("loading");
  const [view, setViewState] = useState<View>(routeView && VALID_VIEWS.has(routeView) ? routeView : "command");
  // A route-supplied mode (e.g. /admin-command vs /worker-command) wins over the
  // remembered preference so the URL always lands on the workspace it names.
  const [profileMode, setProfileMode] = useState<ProfileMode>(() => routeMode ?? (typeof window !== "undefined" && window.localStorage.getItem("grounds-command.profile-mode") === "field" ? "field" : "owner"));
